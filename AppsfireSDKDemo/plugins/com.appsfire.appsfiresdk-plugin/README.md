...
*Arguments:*  
`debugModeEnabled`: Boolean to active/deactive debug mode. It's usefullif you want to test your implementation because it will generate test ads. **Don't forget to de-activate in production!** (Default is `false`)
...
#### Request a modal ad
*Method:*  
`adsdk_requestModalAd()`

*Usage:*  
```js
af.ad.adsdk_requestModalAd();
```

#### Check if there is a modal ad to display
*Method:*  
`adsdk_isThereAModalAdAvailable(callback)`

*Usage:*  
```js
af.adsdk_isThereAModalAdAvailable(function(isThereAModalAdAvailable){
